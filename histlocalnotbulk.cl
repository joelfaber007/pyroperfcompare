...
__kernel void addToHistogramLocalNB(
    __global PyroXY* zin,
    ...
    float pixel_per_unit)
{
  ...
  if (histogram_index > 0)
  {
    ...
    // If this is the minimum local ID writing to any index, accumulate each component of the histogram.
    if (lid == mincacheid)
    {
      histogram[histogram_index].r += cache[mincacheid].r;
      histogram[histogram_index].g += cache[mincacheid].g;
      histogram[histogram_index].b += cache[mincacheid].b;
      histogram[histogram_index].count += cache[mincacheid].count;
    }
  }
}
